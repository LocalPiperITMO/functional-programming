...
*Как можно заметить, в данном коде для отображения используется общая реализация `map-trie-any`, и частная реализация `map-trie-string`. Это связано с тем, что, хотя строки и коллекции являются итерируемыми объектами, в Clojure строки это `java.lang.String`, а коллекции - `clojure.lang.IPersistentCollection`. Это создает определенные неудобства - что поделать, Java есть Java в любой ее обертке. Всегда можно посмотреть в сторону других языков - [вот так, например, выглядит гордый пользователь Rust](https://github.com/owl-from-hogvarts)
